	...
	<div class="wo_tittle-list" id="load-data">
			@foreach ($tvshows as $movie)
				<?php 
					...
					$movieTitleAdwordUrl = '&keyword='.$movieTitle.'&matchtype='.$parameters['matchtype'] ?? "{matcht}" .'&creative= '.$parameters['creative'] ?? "{ad2}".'&gclid='.$parameters['gclid'] ?? "{556688}";
				?>

				<div class="wo_movie-item__link">
					<div class="wo_movie-item__poster">
						<div class="wo_movie-item__poster" onclick="window.location.href='/tvshows/show/{{ $tvId }}?{{ $movieTitleAdwordUrl }}'" style="background-image: url(&quot; {{ $posterImage }} &quot;);">
			
						</div>
						<div class="wo_movie-item__details"><a href="/tvshows/show/{{ $tvId }}?{{ $movieTitleAdwordUrl }}" data-event-name="See details click" data-event-label="Fifty Shades Darker" data-event-category="List Page" class="wo_movie-item__wrap track_event"><div class="wo_movie-item__title">{{ $movie['name'] }}</div><div class="wo_movie-item__info"><span class="wo_movie-item__info__year">{{ date("Y",strtotime($movie['first_air_date'])) }} </span><div class="wo_movie-item__info__rating"><div class="rating">
